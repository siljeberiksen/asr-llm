...
COUNT_FILE = "experiments.experiment_55.count.txt"
...
def run_experiment(result_file, beam_file, wer_file, whisper_model, context_len, tracker,count=0):
    ...
    for true_transcription_data in true_transcriptions_data:
        ...
        if(true_transcription_data['audio'] == last_element["audio_file"] and not last_element_passed):
            ...
            while(len(prev_context) >= context_len):
                prev_context.pop(0)
        ...
        if count < 50:
            if (os.path.isfile(os.path.join("../NPSC/NPSC_1", true_transcription_data['audio']))):
                result = whisper_model.transcribe(f"../NPSC/NPSC_1/{true_transcription_data['audio']}", beam_size=10, without_timestamps=True, context=context, integrate_llm=True, port=8082, experiment_number=55)
            elif (os.path.isfile(os.path.join("../NPSC/NPSC_2", true_transcription_data['audio']))):
                result = whisper_model.transcribe(f"../NPSC/NPSC_2/{true_transcription_data['audio']}", beam_size=10, without_timestamps=True, context=context, integrate_llm=True, port=8082, experiment_number=55)
            elif (os.path.isfile(os.path.join("../NPSC/NPSC_3", true_transcription_data['audio']))):
                result = whisper_model.transcribe(f"../NPSC/NPSC_3/{true_transcription_data['audio']}", beam_size=10, without_timestamps=True, context=context, integrate_llm=True, port=8082, experiment_number=55)
            elif  (os.path.isfile(os.path.join("../NPSC/NPSC_4", true_transcription_data['audio']))):
                result = whisper_model.transcribe(f"../NPSC/NPSC_4/{true_transcription_data['audio']}", beam_size=10, without_timestamps=True, context=context, integrate_llm=True, port=8082, experiment_number=55)
            else:
                result = whisper_model.transcribe(f"../NPSC/NPSC_5/{true_transcription_data['audio']}", beam_size=10, without_timestamps=True, context=context, integrate_llm=True,port=8082, experiment_number=55)
