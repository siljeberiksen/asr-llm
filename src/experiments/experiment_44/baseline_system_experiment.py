...
def run_experiment(result_file, beam_file, wer_file, whisper_model, tracker):
    ...
    for true_transcription_data in true_transcriptions_data:
        ...
        if (os.path.isfile(os.path.join("../NPSC/NPSC_1", true_transcription_data['audio']))):
            result = whisper_model.transcribe(f"../NPSC/NPSC_1/{true_transcription_data['audio']}", beam_size=3, without_timestamps=True, context=context, integrate_llm=False, experiment_number=44)
        elif (os.path.isfile(os.path.join("../NPSC/NPSC_2", true_transcription_data['audio']))):
            result = whisper_model.transcribe(f"../NPSC/NPSC_2/{true_transcription_data['audio']}", beam_size=3, without_timestamps=True, context=context, integrate_llm=False, experiment_number=44)
        elif (os.path.isfile(os.path.join("../NPSC/NPSC_3", true_transcription_data['audio']))):
            result = whisper_model.transcribe(f"../NPSC/NPSC_3/{true_transcription_data['audio']}", beam_size=3, without_timestamps=True, context=context, integrate_llm=False, experiment_number=44)
        elif  (os.path.isfile(os.path.join("../NPSC/NPSC_4", true_transcription_data['audio']))):
            result = whisper_model.transcribe(f"../NPSC/NPSC_4/{true_transcription_data['audio']}", beam_size=3, without_timestamps=True, context=context, integrate_llm=False, experiment_number=44)
        else:
            result = whisper_model.transcribe(f"../NPSC/NPSC_5/{true_transcription_data['audio']}", beam_size=3, without_timestamps=True, context=context, integrate_llm=False, experiment_number=44)
...
tracker = EmissionsTracker(project_name="experiment_44_baseline")
run_experiment('../result/npsc_samtale_experiment_44.json', '../result/beam_npsc_experiment_44.json',"../result/wer_npsc_experiment_44.json", whisper_model, tracker)