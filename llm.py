...
def choose_best_sentence(context, choices):
   ...
   # prompt = "Velg blant de alternative settene og gi meg et svar som passer til kontektsen."
    ...
    # prompt = "You are an ASR transcript selector. You are analyzing five beam search transcription outputs from an automatic speech recognition system. Your task is to generate the most likely transcript from them given a context."
    # prompt += f"Context: {', '.join(context)}"
    # for i, choice in enumerate(choices, 1):
    #      prompt += f"<option{i}> {choice} </option{i}>"
    # prompt += "Please output the selected best ASR beam search transcription output as <option?> the selected selected best ASR beam search transcription </option?>" 
